...
import { getAllDeans, saveDean, updateDean, deleteDean } from "../../../services/deanService";
...
const DeanPage = () => {
    ...
    const load = async () => {
        setLoading(true);
        try { const res = await getAllDeans(); setData(res.data || []); }
        catch { setData([]); }
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm, setSubmitting }) => {
            try {
                if (editId) await updateDean(editId, values);
                else await saveDean(values);
                resetForm(); setShowModal(false); load();
                Swal.fire({ icon: "success", title: editId ? "Dean updated!" : "Dean saved!", timer: 1500, showConfirmButton: false });
            } catch (err) {
    ...
    const handleDelete = (id) => {
        Swal.fire({ title: "Delete this dean?", icon: "warning", showCancelButton: true, confirmButtonColor: "#dc2626", confirmButtonText: "Yes, Delete" })
            .then(async (r) => {
                if (!r.isConfirmed) return;
                try { await deleteDean(id); load(); }
    ...
    return (
        <div>
            <div className="page-header">
                <h4>Dean Management</h4>
                <button className="btn btn-primary d-flex align-items-center gap-2" onClick={openAdd}><FiPlus size={15} /> Add Dean</button>
            </div>
            <div className="table-card">
                <div className="p-3 border-bottom">
                    <div className="input-group" style={{ maxWidth: 320 }}>
                        <span className="input-group-text bg-white border-end-0"><FiSearch size={14} color="#64748b" /></span>
                        <input className="form-control border-start-0" placeholder="Search deans..." value={filter} onChange={(e) => setFilter(e.target.value)} />
                    </div>
                </div>
                <DataTable columns={columns} data={filtered} progressPending={loading} pagination dense highlightOnHover responsive noDataComponent={<div className="p-4 text-muted">No deans found.</div>} />
            </div>
            <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered>
                <Modal.Header closeButton><Modal.Title>{editId ? "Edit Dean" : "Add New Dean"}</Modal.Title></Modal.Header>
                <Modal.Body>
                    <form id="deanForm" onSubmit={formik.handleSubmit} noValidate>
                ...
                <Modal.Footer>
                    ...
                    <button className="btn btn-primary" form="deanForm" type="submit" disabled={formik.isSubmitting}>
                        {formik.isSubmitting ? "Saving..." : editId ? "Update Dean" : "Save Dean"}
                    </button>
                </Modal.Footer>
            </Modal>
        </div>
